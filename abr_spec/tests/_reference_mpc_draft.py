"""FROZEN pre-refactor copy of plm_special/speculative/mpc_draft.py.

Do not edit and do not import outside abr_spec/tests/.  This is the byte-for-byte
generator as it stood at commit b6ebf8e, kept only so
``test_drafters.py::MPCRefactorRegressionTest`` can prove that splitting the
proposer from the trajectory simulator did not change a single drafted action,
state, return, buffer, reward or rebuffer value on the ``mpc`` path.

Regenerate with:
    git show <pre-refactor-commit>:adaptive_bitrate_streaming/plm_special/speculative/mpc_draft.py \
      > abr_spec/tests/_reference_mpc_draft.py
