...
| 6 | Whatever rebuffering a drafter does add is concentrated in a **small number of queue entries that execute after the buffer drains** (the drafter cannot see the future drain), not spread across the queue serves. | Fig. 4(left/band); Fig. 5(right); DRAFTER_ABLATION §S.6, §9.13 | queue-serve rebuffer-event incidence ≤ 0.97× the mpc control; ≥ 20 s buffer band (76 % of queue serves) contributes 0 s; `< 5 s` band carries ~all of it. At 4 seeds the k=3 net Δrebuffering is inside the A1 seed band (claim 3b) — so this is now a *mechanism* statement, not a "cost" | 4,700 decisions per-band (seed 1 mechanism) + 4-seed net effect | the *identity* of the few entries is seed-dependent; the mechanism (drain-after-draft) is not |
