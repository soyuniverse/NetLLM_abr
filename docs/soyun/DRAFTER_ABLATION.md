...
## 결론 — **[drafter 교체로 speedup 1.24× 벽 돌파; 비용은 4-seed 로 확정]**

파라미터로는 못 넘던 **speedup 1.0·1.24× 선을 넘었다.** `repeat-last` drafter
(직전 행동을 k번 반복) 가 LLM 의 다음 행동을 step-1 에서 **88 %** 맞혀 (mpc 는
13–16 %), 재사용률 q 를 7 % → **38 %** 로 밀어 올린다. **이 헤드라인은 seed 4개
(1–4) 평균으로 재확증됐다** (§9.13) — q 는 σ < 0.7 pp 로 극도로 안정적이며 RNG
스트림 위치와 무관하다 ([[RNG_CONTAMINATION_AUDIT]] §3).

| drafter | speedup | q | ΔQoE % vs 같은-seed A1 | Δrebuffering s |
|---|---:|---:|---:|---:|
| mpc (대조군, k3) — seed 1 / reseed | 0.97× / 1.07× | 7.4 % / 7.0 % | +0.8 / −1.5 | +1.1 / +32 |
| **repeat-last k3 (m2)** — 4-seed | **1.49 ± 0.08×** | **38.2 ± 0.3 %** | **+0.4 ± 2.1** | **+5 ± 23** |
| hybrid k3 (m3) — 4-seed | 1.50 ± 0.08× | 37.1 ± 0.3 % | −0.5 ± 2.5 | +6 ± 12 |
| repeat-last k5 (m4) — 4-seed | 1.46 ± 0.12× | 42.6 ± 0.6 % | −0.8 ± 2.3 | +15 ± 22 |
| hybrid k5 (m5) — 4-seed | 1.48 ± 0.11× | 42.0 ± 0.7 % | +0.5 ± 3.4 | −5 ± 22 |
| **repeat-last k3 + Temporal/Token selector (M6)** — 4-seed | **2.03 ± 0.10×** | 37.8 ± 0.2 % | **−3.0 ± 1.9** | **+38 ± 29** |

<sub>speedup std 는 대부분 seed 1 (구 세션, 지연 ~57 ms) vs seed 2–4 (~50 ms) 의
세션 간 드리프트; q 는 세션 무관하게 안정적이다. mpc 는 seed 2–4 미실행 (Task 1
seed-1 + reseed 검정만).</sub>

**지난 세션의 "조건부 성공 / rebuffering 이 A1 의 2.9–5.8×" 는 seed 1 아티팩트였다**
(§9.13). rebuffering 총량은 **A1 을 포함한 모든 조건에서** 매 seed 1–4개 trace 가
지배하고(top-1 trace 가 총량의 32–100 %), seed 간 σ 가 20 s 안팎이다 — A1 baseline
자체가 6.4 / 15.2 / 12.7 / 33.3 s 로 흔들린다. **k=3 zero-search/hybrid drafter 의
ΔQoE·Δrebuffering 은 4-seed 에서 0 과 구분되지 않는다.** 유일하게 통계적으로
유의한 비용은 **M6** (ΔQoE −3.0 ± 1.9 %, Δrebuffering +38 ± 29 s) — 선택기를
얹어 2× 를 얻는 대신 실제 QoE 를 내준다.

**배치 관점 (§9.13):** ~1.5× 를 QoE 중립으로 원하면 **repeat-last 또는 hybrid,
k=3**. 2× 가 ~3 % QoE 저하보다 중요하면 **M6**. **k=5 는 전 축에서 열등** (speedup
낮고 rebuffering 높음). 안전 지표(§S)의 사전 임계 판정은 seed 1 기준이었으므로
§9.13 의 4-seed 수치로 대체한다.
...
rebuffering A1 의 2.9–5.8×" 는 seed 1 아티팩트였고, **§9.13 에서 4-seed 로
재측정해 확정판으로 대체했다** (헤드라인 speedup·q 는 재확증, 비용은 0 과
구분 불가, M6 만 유의). 원인은 공유 RNG 스트림 + rebuffering 을 1–2개 trace 가
지배하는 구조 ([[TRAJECTORY_DIVERGENCE]] · [[RNG_CONTAMINATION_AUDIT]] — 이번
연구의 방법론적 핵심 성과).
...
**~~이월 (HANDOFF Action 5)~~ → 완료 §9.13 (2026-09-10).** m2·m3·m4·m5·M6 를
seed 2–4 로 재실행했다. 결과: 헤드라인(speedup·q)은 4-seed 로 재확증, 조건부
성공의 비용 부분은 0 과 구분 불가, M6 만 유의한 QoE 비용. seed 민감도의 원인
(공유 RNG 스트림 + rebuffering 을 1–2개 trace 가 지배)은 [[TRAJECTORY_DIVERGENCE]]
§3 · [[RNG_CONTAMINATION_AUDIT]].
...
### 9.13 G1 — drafter ablation 4-seed 확정 (2026-09-10)

`A1 / m2 / m3 / m4 / m6` 를 seed {2,3,4} 로 재실행
(`results/soyun/drafter_seed_sweep_20260910/`, `abr_spec/run_drafter_seed_sweep.sh`,
실행경로 == ablation freeze `0d137ce`). seed 1 은 `drafter_ab_20260908` 재사용,
m5 seed 2–4 는 `serve_gate_20260909/m5_ctrl_s{2,3,4}` 재사용 (게이트
`patched:false`, drafter 경로 바이트 동일). 집계: `abr_spec/build_seed_sweep_report.py`
→ `seed_sweep_report.txt`. **이것이 §2 "조건부 성공" 판정을 대체하는 확정판이다.**

#### (1) seed별 원자료

| cond | seed | QoE | ΔQoE% | rebuf s | Δrebuf s | speedup | q% | 1-step% | top-1 trace 지분 | nz(>0.5s) |
|---|--:|--:|--:|--:|--:|--:|--:|--:|--:|--:|
| A1 | 1/2/3/4 | .949/.927/.948/.919 | — | 6.4/15.2/12.7/33.3 | — | 1.00 | 0 | — | 48/87/40/39 % | 3/3/3/4 |
| m2 rl k3 | 1/2/3/4 | .934/.924/.950/.951 | −1.5/−0.4/+0.2/+3.4 | 23.9/42.2/13.8/8.2 | +17/+27/+1/−25 | 1.42/1.43/1.53/1.58 | 37.8/38.4/38.1/38.3 | 88.0/88.0/89.3/88.6 | 44/31/47/97 % | 3/8/3/1 |
| m3 hy k3 | 1/2/3/4 | .932/.950/.919/.923 | −1.8/+2.5/−3.1/+0.4 | 26.9/11.3/25.3/28.7 | +20/−4/+13/−5 | 1.41/1.45/1.55/1.59 | 37.3/36.9/36.7/37.4 | 83.2/84.0/82.0/82.8 | 48/53/95/100 % | 4/2/2/1 |
| m4 rl k5 | 1/2/3/4 | .911/.916/.960/.925 | −4.0/−1.3/+1.2/+0.6 | 37.1/52.3/2.0/37.9 | +31/+37/−11/+5 | 1.31/1.45/1.55/1.55 | 41.9/42.4/43.1/43.0 | 87.2/87.7/87.3/86.8 | 75/88/100/57 % | 3/3/1/4 |
| m5 hy k5 | 1/2/3/4 | .930/.956/.920/.953 | −2.0/+3.1/−2.9/+3.7 | 18.5/4.2/25.6/0.5 | +12/−11/+13/−33 | 1.33/1.52/1.56/1.53 | 41.9/42.1/42.7/41.1 | 82.7/82.3/84.2/81.3 | 71/50/41/100 % | 3/3/5/0 |
| m6 rl k3+sel | 1/2/3/4 | .912/.880/.927/.913 | −3.9/−5.0/−2.2/−0.7 | 30.4/96.5/38.0/53.2 | +24/+81/+25/+20 | 2.10/1.90/2.00/2.11 | 37.8/37.8/37.6/38.0 | 86.2/85.6/86.7/86.0 | 51/38/52/85 % | 3/4/4/4 |

#### (2) mean ± std (n=4)

| cond | QoE | ΔQoE % | rebuf s | **Δrebuf s** | speedup | **q %** | 1-step % |
|---|--:|--:|--:|--:|--:|--:|--:|
| A1 | .936 ± .015 | — | 16.9 ± 11.6 | — | 1.00 | 0 | — |
| **m2 rl k3** | .940 ± .013 | **+0.4 ± 2.1** | 22.0 ± 14.9 | **+5.1 ± 22.8** | 1.49 ± 0.08 | **38.2 ± 0.3** | 88.5 ± 0.6 |
| m3 hy k3 | .931 ± .014 | −0.5 ± 2.5 | 23.0 ± 8.0 | +6.1 ± 12.4 | 1.50 ± 0.08 | 37.1 ± 0.3 | 83.0 ± 0.9 |
| m4 rl k5 | .928 ± .022 | −0.8 ± 2.3 | 32.3 ± 21.4 | +15.4 ± 22.4 | 1.46 ± 0.12 | 42.6 ± 0.6 | 87.3 ± 0.4 |
| m5 hy k5 | .940 ± .018 | +0.5 ± 3.4 | 12.2 ± 11.8 | −4.7 ± 21.8 | 1.48 ± 0.11 | 42.0 ± 0.7 | 82.6 ± 1.2 |
| **m6 rl k3+sel** | .908 ± .020 | **−3.0 ± 1.9** | 54.5 ± 29.5 | **+37.6 ± 29.2** | 2.03 ± 0.10 | 37.8 ± 0.2 | 86.1 ± 0.5 |

#### (3) 판독

- **헤드라인은 4-seed 로 재확증 (Task 1 판정 강화).** q 는 σ 0.2–0.7 pp,
  1-step 은 σ < 1.2 pp — RNG 스트림·seed 와 무관. 모든 drafter 가 1.24× 를
  넘는다: k3 zero-search/hybrid ~1.5×, k5 ~1.46–1.48×, **M6 ~2.03×**. mpc 는
  ~1.0× (Task 1). speedup 자체의 σ(~0.1)는 대부분 seed 1(구 세션)–seed 2–4 지연
  드리프트이고 q 는 안정적이다.
- **"조건부 성공"의 비용 부분은 4-seed 에서 사라진다.** m2/m3/m5 의 ΔQoE·Δrebuf
  는 0 을 포함하는 큰 신뢰구간(±2–3 %, ±12–23 s). "seed 1 rebuffering 2.9–5.8×
  A1" 은 그 seed 의 불운한 trace 조합이었다.
- **이상치 지배는 A1 포함 전 조건에서 나타난다** — 매 run 의 rebuffering 을
  top-1 trace 가 32–100 % 차지하고, >0.5 s rebuffer trace 는 100개 중 1–8개.
  **제거하지 않는다** (원본 유지). trimmed(top-1 제외) 총량은 대부분 0–15 s 로
  줄지만, 이는 "구조적 비용은 작다"는 정성적 근거일 뿐 지표로 쓰지 않는다 —
  어느 trace 가 이상치인지가 seed 가 정하므로.
- **유일하게 유의한 비용은 M6.** ΔQoE −3.0 ± 1.9 % (4개 seed 전부 음수),
  Δrebuffering +38 ± 29 s. 선택기가 2× 를 주지만 QoE 를 실제로 깎는다. m6 seed 2
  는 rebuffering 96 s (trace 36·13·74 세 개가 88 s) — n=1 이었으면 못 봤을 값.
- **k=5 는 열등.** m4(rl k5) Δrebuf +15 ± 22 vs m2(rl k3) +5 ± 23, speedup 은
  낮음(q 는 더 높지만 c_verify 증가가 상쇄). m5(hy k5)만 Δrebuf −5 이나 σ 22,
  k3 대비 speedup 이점 없음.

#### (4) 배치 관점 최종 요약 (§2 "조건부 성공" 대체)

**Zero-search 또는 hybrid drafter, k=3** — LLM 호출 ~1.5× 절감 (q 37–38 %,
4-seed σ < 0.3 pp), **4-seed 에서 QoE·rebuffering 비용이 0 과 구분되지 않음**.
repeat-last(m2)와 hybrid(m3)는 사실상 동등 (hybrid 가 rebuf 분산 약간 낮음).
**2× 가 필요하면 M6**, 단 ΔQoE −3 % 를 감수. **k=5 는 채택하지 않는다.**
mpc drafter 는 speedup 이 없다 (~1.0×).
